...
from src.water_scale_model import WaterScaleImageProcessor
...
if not os.path.exists(os.path.join(config.get("image", "save_path"))):
    os.mkdir(os.path.join(config.get("image", "save_path")))
if not os.path.exists(os.path.join(config.get("water_level", "save_path"))):
    os.mkdir(os.path.join(config.get("water_level", "save_path")))
...
def process_regions_controller(base64_image):
    if base64_image:
        if is_file(os.path.join(config.get("image", "image_path"), base64_image)):
            ...
            file_name = base64_image.split(".")[0] + f"_{int(timestamp)}_result.jpg"
            result_path = os.path.join(
                ROOT, config.get("image", "save_path"), file_name
            )
...
# water scale
def water_level_controller(image_data):
    if image_data:
        if is_file(os.path.join(config.get("water_level", "image_path"), image_data)):
            timestamp = time.time()
            file_name = image_data.split(".")[0] + f"_{int(timestamp)}_result.jpg"
            image_path = os.path.join(
                config.get("water_level", "image_path"), image_data
            )
            image_path = os.path.join(ROOT, image_path)
            result_path = os.path.join(
                ROOT, config.get("water_level", "save_path"), file_name
            )

            img = cv2.imread(image_path, cv2.COLOR_BGR2RGB)

            image_processor = WaterScaleImageProcessor(
                image=img, exported_file=result_path
            )
            result = image_processor.process_image()
            if not result:
                return {"success": False}
            return {
                "success": True,
                "original_file": image_data,
                "result_file": file_name,
                "water_level": result["water_level"],
            }
        else:
            temp_path = config.get("water_level", "image_path")
            save_path = config.get("water_level", "save_path")
